...
// https://stackoverflow.com/questions/28574685/setcookie-with-javascript
function setCookie(name,value,days) {
    ...
    document.cookie = name+"="+value+expires+"; path=/mzgb/backoffice";
}

// https://stackoverflow.com/questions/4003823/javascript-getcookie-functions
function getCookies() {
...
// to be defined in php:
//var ${entity}_columnIndexes_forViewing = [9, 10];
//var ${entity}_columnIndexes_forEditing = [1, 2, 6, 11, 12];
var inputs_forEditing_only = ['input#btn_save_list', 'input#btn_deleteChecked_list', 'input#cb_checkall_4delete', 'label[for=cb_checkall_4delete]'];

var cookieExpires_inDays = 30;
var cookieExpires_inMillis = cookieExpires_inDays * 24 * 60 * 60 * 1000;

function showColumns_changed(entity, columnIndexes_forViewing, columnIndexes_forEditing) {
	var whichRadioChecked_selector = "form#showColumns input[type=radio]:checked";
	//debugger;
	var whichRadioChecked = $(whichRadioChecked_selector);
	var id_checked = $(whichRadioChecked ).attr("id");
	if (id_checked == 'showColumns_forViewing') {
		showColumns_forViewing_selected(entity, columnIndexes_forViewing, columnIndexes_forEditing);
	} else {
		showColumns_forEditing_selected(entity, columnIndexes_forViewing, columnIndexes_forEditing);
	}
}
function showColumns_forViewing_selected(entity, columnIndexes_forViewing_showAll, columnIndexes_forEditing_hideAll) {
	$.each(columnIndexes_forViewing_showAll, function(arrayIndex, columnIndex) {
		$('table.gw td:nth-child(' + columnIndex + ')').show();
		$('table.gw th:nth-child(' + columnIndex + ')').show();
	});
	
	$.each(columnIndexes_forEditing_hideAll, function(arrayIndex, columnIndex) {
		$('table.gw td:nth-child(' + columnIndex + ')').hide();
		$('table.gw th:nth-child(' + columnIndex + ')').hide();
	});
	
	$.each(inputs_forEditing_only, function(arrayIndex, inputToHide) {
		$(inputToHide).hide();
	});
	
	$("form#showColumns input#showColumns_forViewing").attr("checked", true);
	
	//if (getCookie('question_showColumns') == 'viewing') return;
	setCookie(entity + '_showColumns', 'viewing', cookieExpires_inDays);
	var qc = getCookie(entity + '_showColumns');
}
function showColumns_forEditing_selected(entity, columnIndexes_forViewing_hideAll, columnIndexes_forEditing_showAll) {
	$.each(columnIndexes_forViewing_hideAll, function(arrayIndex, columnIndex) {
		$('table.gw td:nth-child(' + columnIndex + ')').hide();
		$('table.gw th:nth-child(' + columnIndex + ')').hide();
	});
	
	$.each(columnIndexes_forEditing_showAll, function(arrayIndex, columnIndex) {
		$('table.gw td:nth-child(' + columnIndex + ')').show();
		$('table.gw th:nth-child(' + columnIndex + ')').show();
	});
	
	$.each(inputs_forEditing_only, function(arrayIndex, inputToShow) {
		$(inputToShow).show();
	});

	$("form#showColumns input#showColumns_forEditing").attr("checked", true);
	
	//if (getCookie('question_showColumns') == 'editing') return;
	setCookie(entity + '_showColumns', 'editing', cookieExpires_inDays);
	var qc = getCookie(entity + '_showColumns');
}