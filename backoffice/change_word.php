...
//$skip_tables = array(TABLE_PREFIX + "sentlog");
$skip_tables = array();
...
*/
	...
	for ($i = 0; $i < mysql_num_rows($dbtables_result); $i++) {
	    ...
	    if ($prefix_position !== 0) {
			$dbtables_scanned .= "<li>$dbtable NOT_PREFIXED_WITH[" . TABLE_PREFIX . "]</li>\n\r";
			continue;
		}
	    if (in_array($dbtable, $skip_tables)) {
			$dbtables_scanned .= "<li>$dbtable [SKIPPED]</li>\n\r";
			continue;
		}
		//pre($dbtable);
...
//		break;
		...
		$dbtables_scanned .= "<li>$dbtable</li>\n\r";
...
if ($mode != "" && $kw_search != "" && $kw_replace != "") {
	if ($process_log == "") {
		...
		$dbtables_scanned = "<ol>$dbtables_scanned</ol>";
		$process_log .= "<tr><td colspan=5>$dbtables_scanned</b></td></tr>";
	} else {
		$process_log .= <<< EOT
...
<tr valign=top>
	...
	<caption>желательно без пробелов и кавычек</caption>
	<tr>
