...
//	$entity = prefixed_entity ($entity);
	...
	if (!$result) {
		die("SELECT_QUERYARRAY failed:<br>$query<br>" . mysql_error($cms_dbc));	
	}
...
//	pre("$ret mysql_affected_rows($query)");
...
function select_firstRow_byQuery($query, $invoker = "INVOKER_UNKNOWN") {
	...
	$result = mysql_query($query, $cms_dbc) or die("$invoker failed:<br>$query<br>" . mysql_error($cms_dbc));
	if (mysql_num_rows($result) == 0) {
		return array();
	}
...
//	$entity = prefixed_entity ($entity);
	...
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		if (isset($row["id"])) {
			if ($row["id"] == $current) {
				$item_tmp = $tpl_current;
				//echo "[" . $row ["id"] . ":current] ";
			}
			if ($entity == "mmenu" && $row["hashkey"] == "=mmenu.php?id=$current") {
				// submenu in IBSfreight
				$item_tmp = $tpl_current;
				//echo "[" . $row ["id"] . ":current] ";
			}
		}
		...
		$row["i"] = ++$i;
		if (!isset($row["entity"])) $row["entity"] = $entity;
		//echo "[" . $row ["id"] . ":" . $row ["i"] . "|" . $row["rows_total"] . "] ";
...
//	$entity = prefixed_entity ($entity);
	...
	$query = add_sql_table_prefix($query);
	if ($debug_query == 1) echo "<br>SELECT ENTITY_TPL[$query]<br>";
...
function tree_tpl($tpl, $tpl_current = "_same", $entity = "_global:entity", $current = "_global:id"
	...
	, $level = 1, $level_stop = 0, $down_wrapper_tpl = "#DOWN_CONTENT#", $downempty_wrapper_tpl = ""
	, $parent_class_when_has_children = "sub-menu", $query_tpl = "", $cms_dbc = "_global:cms_dbc") {
...
//	$entity = prefixed_entity ($entity);
	...
	while($row = mysql_fetch_assoc($result)) {
		...
		$row["down_content"] = "";
		$row["parent_class_when_has_children"] = "";
		
		if ($level_stop == 0 || $level <= $level_stop) {
			$down_content = tree_tpl($tpl, $tpl_current, $entity, $current, $down_id, $fixed_hash,
							   $level+1, $level_stop, $down_wrapper_tpl, $downempty_wrapper_tpl,
							   $parent_class_when_has_children, $query_tpl);

			if ($down_content != "") {
				if ($down_wrapper_tpl != "") {
					$down_hash = array("down_content" => $down_content, "parent_class_when_has_children" => "");
					$down_content = hash_by_tpl($down_hash, $down_wrapper_tpl);
				}
				//echo "[$down_content] ";
				$row["down_content"] = $down_content;
				$row["parent_class_when_has_children"] = $parent_class_when_has_children;
			} else {
				//"processing anyway" contributes to injections like
				//$down_wrapper_tpl = "<ul>#DOWN_CONTENT#@inject_pgrouptree@</ul>";
				$down_hash = $row;
				$row["down_content"] = hash_by_tpl($down_hash, $downempty_wrapper_tpl);
			}
		}
...
function m2mtfcontrolled_update($dict_table, $fieldshash_byid, $m2m_tablename_4update
		, $m2m_fixed4select, $m2m_fixed4insert
//		, $m2m_hash_base = array($entity => $id)
		) {
	global $entity, $id, $debug_query, $cms_dbc;
	$answers_inserted = 0;
	$answers_updated = 0;
	...
	$ret = "";
	$msig = " //m2mtfcontrolled_update($dict_table)";

//	pre("m2mtfcontrolled_update(dict_table[$dict_table]"
//		. ", fieldshash_byid[" . pr($fieldshash_byid) . "], m2m_tablename_4update[$m2m_tablename_4update]"
//		. ", m2m_fixed4select[" . pr($m2m_fixed4select) . "]"
//		. ", m2m_fixed4insert[" . pr($m2m_fixed4insert) . "]"
//		);
...
//	$debug_query = 1;
	if (is_array($fieldshash_byid) == false) {
		pre("FORM_WAS_NOT_SUBMITTED $msig");
		return;
	}
	
	foreach ($fieldshash_byid as $dict_id => $m2m_hash_submitted) {
		$m2m_hash_forThisCheckbox = array($dict_table => $dict_id);
		//$m2m_hash_forThisCheckbox = array_merge($m2m_hash_base, $m2m_hash_forThisCheckbox);
		$m2m_hash_select = array_merge($m2m_hash_forThisCheckbox, $m2m_fixed4select);
//		pre($m2m_hash_select, "m2m_hash_select");
		$m2m_row = select_entity_row($m2m_hash_select, $m2m_tablename_4update);

		//pre(pr($m2m_hash_submitted, "m2m_hash_submitted_PRE_CHECKBOX") . " for " . pr($m2m_row, "m2m_row"));
		$m2m_hash_submitted = checkboxes_submitted_to_updatehash($m2m_hash_submitted, array("correct"));
		//pre(pr($m2m_hash_submitted, "m2m_hash_submitted_POST_CHECKBOX") . " for " . pr($m2m_row, "m2m_row"));

		if (count($m2m_row) == 0) {
			$m2m_insert_hash = array_merge($m2m_hash_select, $m2m_fixed4insert, $m2m_hash_submitted);
			$m2m_insert_hash["date_created"] = "CURRENT_TIMESTAMP";
//			pre($m2m_insert_hash, "m2m_insert_hash");
			$m2m_row_id = insert($m2m_insert_hash, $m2m_tablename_4update);
			$answers_inserted++;
			continue;
		}
		
		$m2m_row_id = $m2m_row["id"];
//		pre($m2m_hash_submitted, "m2m_hash_submitted");
		$updated_rows = update($m2m_hash_submitted, array("id" => $m2m_row_id), $m2m_tablename_4update);
		$answers_updated += $updated_rows;
	}
	
	$ret = "$answers_inserted:$answers_updated";
	return $ret;
}

function m2mtfcontrolledgrouped_update($table_inside_group_ledby_m2m_controlling_table = "team",
		$namedhash_fieldsbyid = array(
			/*"team" => array (						// outer_branch
				2 => array (						// outer_id => inner_branch_by_outer_id
					"question" => array (			// inner_branch
						35 => array (				// inner_id => m2m_hash_submitted
							"correct" => "on",		// mandatory checkbox
							"content" => "1234"
						),
						36 => array (
							"content" => ""
		...
		, $m2m_tablename_4update = "m2m_team_question"
		, $m2m_fixed4insert_content = array("game" => "_global:game", "tour" => "_global:tour")
		, $inner_table = "question"
//		, $m2m_fixed4insert = array("game" => "_global:game", "tour" => "_global:tour")
	) {
	global $entity, $id, $debug_query, $cms_dbc, $_REQUEST;
	$ret = "";
	$msig = " //m2mtfcontrolledgrouped_update($m2m_tablename_4update)";
	
/*	pre("table_inside_group_ledby_m2m_controlling_table["
			. $table_inside_group_ledby_m2m_controlling_table . "]"
		. ", namedhash_fieldsbyid[" . pr($namedhash_fieldsbyid) . "]"
		. ", m2m_tablename_4update[$m2m_tablename_4update]"
		. ", m2m_fixed4insert_content[" . pr($m2m_fixed4insert_content) . "]"
		. ", inner_table[$inner_table]"
		, $msig);
*/

	if (isset($namedhash_fieldsbyid[$table_inside_group_ledby_m2m_controlling_table]) == false) {
		pre("FORM_WAS_NOT_SUBMITTED $msig");
		return;
	}
	$outer_branch = $namedhash_fieldsbyid[$table_inside_group_ledby_m2m_controlling_table];	// team
	
	foreach($outer_branch as $outer_id => $inner_branch_by_outer_id) {		// "2" => (question => 35 => content = "1234")
		$inner_branch = $inner_branch_by_outer_id[$inner_table];
		foreach ($inner_branch as $inner_id => $m2m_hash_submitted) {
			$m2m_hash_base = array(
				//$entity => $id,														// (tour => 8)
				$table_inside_group_ledby_m2m_controlling_table => $outer_id,			// (team => 2)
				$inner_table => $inner_id);												// (team => 2, question => 35)
			$m2m_hash_select = array_merge($m2m_hash_base, $m2m_fixed4insert_content);	// (team => 2, question => 35, game => 2, tour => 8)
			$m2m_row = select_entity_row($m2m_hash_select, $m2m_tablename_4update);

			//pre(pr($m2m_hash_submitted, "m2m_hash_submitted_PRE_CHECKBOX") . " for " . pr($m2m_row, "m2m_row"));
			$m2m_hash_submitted = checkboxes_submitted_to_updatehash($m2m_hash_submitted, array("correct"));	// correct => 1, content => 1234
			//pre(pr($m2m_hash_submitted, "m2m_hash_submitted_POST_CHECKBOX") . " for " . pr($m2m_row, "m2m_row"));

			if (count($m2m_row) == 0) {
				$m2m_insert_hash = array_merge($m2m_hash_select, $m2m_hash_submitted);
				$m2m_insert_hash["date_created"] = "CURRENT_TIMESTAMP";
				$m2m_row_id = insert($m2m_insert_hash, $m2m_tablename_4update);
				//pre($m2m_insert_hash, "inserted m2m_row_id[$m2m_row_id] m2m_tablename_4update[$m2m_tablename_4update]");
				continue;
			}
		
			$m2m_row_id = $m2m_row["id"];
			update($m2m_hash_submitted, array("id" => $m2m_row_id), $m2m_tablename_4update);
			//pre($m2m_hash_submitted, "updated m2m_row_id[$m2m_row_id] m2m_tablename_4update[$m2m_tablename_4update]");
		}
	}
	
}

function checkboxes_submitted_to_updatehash($form_hash = array(), $checkboxes_names = array(), $value_checked = "on") {
/*		Array (
            [correct] => on
            [content] => 123
        )
...
*/
	$ret = $form_hash;
	foreach ($checkboxes_names as $checkbox_mandatory) {
		if (isset($form_hash[$checkbox_mandatory]) == false) {
			$ret[$checkbox_mandatory] = 0;
			continue;
		}
		$value_int = 0;		// checkbox: "on" when checked; nothing when unchecked => forcing its presence in ret
		if ($form_hash[$checkbox_mandatory] == $value_checked) $value_int = 1;
		$ret[$checkbox_mandatory] = $value_int;
	}
	return $ret;
}
...
//						echo "multiflatcontent($name, $form_value, $entity_field[2])";
				...
				case "multicompositecontent":
					...
					if ($form_value == "") $form_value = array();
					pre("multicompositecontent($entity_field[2], form_value[" . pr($form_value) . "], composite[" . pr($composite) . ")");
					multicompositecontent_update($entity_field[2], $form_value, $composite);
					break;

				//mzgb
				case "m2mtfcontrolled":
					$m2m_tablename_4update = $entity_field[2];	//m2m_team_question
					//USED_ONLY_FOR_RENDERING $m2m_controlling_table = $entity_field[3];	//m2m_game_team
					$m2m_fixed4select = $entity_field[4];		//m2m_team_question_fixed4select
					$m2m_fixed4insert = $entity_field[5];		//m2m_team_question_fixed4insert
...
*/					
					...
					//$m2m_base = array($entity => $id); // question=>35
					m2mtfcontrolled_update($name, $form_value, $m2m_tablename_4update,
						$m2m_fixed4select, $m2m_fixed4insert
						//, $m2m_base
						);
					break;
				...
				//mzgb
				case "m2mtfcontrolledgrouped":
					$m2m_tablename_4update = $entity_field[2];	//m2m_team_question
					//USED_ONLY_FOR_RENDERING $m2m_controlling_table = $entity_field[3];	//m2m_game_team
					//USED_ONLY_FOR_RENDERING $m2m_fixed4select_outer = $entity_field[4];		//m2m_team_question_fixed4select
					$table_inside_group_ledby_m2m_controlling_table = $entity_field[5];		//team
					$m2m_fixed4insert_content = $entity_field[6];		//m2m_team_question_fixed4insert
...
*/					
					//$name="question" here
					if (isset($_REQUEST[$table_inside_group_ledby_m2m_controlling_table])) {
						$form_value = array(
									  $table_inside_group_ledby_m2m_controlling_table =>
							$_REQUEST[$table_inside_group_ledby_m2m_controlling_table]);
//						pre($_REQUEST[$table_inside_group_ledby_m2m_controlling_table], "_REQUEST[$table_inside_group_ledby_m2m_controlling_table]");
					}

					if ($form_value == "") $form_value = array();
/*					pre("m2mtfcontrolledgrouped_update($table_inside_group_ledby_m2m_controlling_table, form_value[" . pr($form_value) . "], $m2m_tablename_4update"
						. ", m2m_fixed4select[" . pr($m2m_fixed4select) . "]"
						. ", table_inside_group_ledby_m2m_controlling_table[" . pr($table_inside_group_ledby_m2m_controlling_table) . "]"
						. ", m2m_fixed4insert[" . pr($m2m_fixed4insert) . "]"
						);
*/

					m2mtfcontrolledgrouped_update(
						$table_inside_group_ledby_m2m_controlling_table,
						$form_value,
						$m2m_tablename_4update,
						$m2m_fixed4insert_content,
						$name
						//$m2m_fixed4insert
						);
					break;
...
function select_person_id_by_auth($auth, $entity = "person", $field = "auth") {
	$ret = select_field("id", array("$field" => $auth), $entity);
	$ret = is_numeric($ret) ? $ret : 0;
	return $ret;
}
...
//		$tpl_multiple = "<a href='#ENTITY#.php?#FNAME_STRICT#=#ID#'>#IDENT_TRUNCATED#</a>";
		$tpl_single = "<a href='#FNAME_STRICT#-edit.php?id=#ID#'>#IDENT#</a>";
		$tpl_multiple = "<a href='#FNAME_STRICT#-edit.php?id=#ID#'>#IDENT_TRUNCATED#</a>";
	}
