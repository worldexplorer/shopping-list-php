...
function m2mtfcontrolled($dict_table, $value = "_global", $m2m_content_table = "m2m_team_question"
		, $m2m_controlling_table = "m2m_game_team"
		, $m2m_controlling_fixedhash = array ("game" => "_global:game")
		, $additional_fields_fixedhash = array()) {
	global $entity, $id, $m2m_it, $cms_dbc, $input_size, $tpl_controlled, $debug_query;
	$ret = "";
	
//	pre("m2mtfcontrolled(dict_table[$dict_table], value[" . pr($value) . "],"
//		. " m2m_content_table[$m2m_content_table]"
//		. " m2m_controlling_table[$m2m_controlling_table]"
//		. " m2m_controlling_fixedhash[" . pr($m2m_controlling_fixedhash) . "]"
//		. " additional_fields_fixedhash[" . pr($additional_fields_fixedhash) . "]"
//		);
...
//		. " order by d." . get_entity_orderby($dict_table);
	...
	$questionnaire = strpos($m2m_content_table, "question") >= 0;
	$questionnaire_fields = $questionnaire ? ", m2m.correct as m2m_correct" : "";
	
	$additional_fields_csv = "";
	foreach ($additional_fields_fixedhash as $fieldname => $alias) {
		$additional_fields_csv .= ", $fieldname as $alias";
	}
	...
	while ($row = mysql_fetch_assoc($result)) {
		...
		//if ($questionnaire) {
		if (isset($row["m2m_correct"])) {
			$row["m2m_correct_checked"] = $row["m2m_correct"] == "1" ? "checked" : "";
		}
...
function m2mtfcontrolledgrouped($dict_table = "question", $value = "_global:id", $m2m_content_table = "m2m_team_question"
		, $m2m_controlling_table = "m2m_game_team"
		, $m2m_controlling_fixedhash = array ("game" => "_global:game")
		, $grouping_table = "team"
		, $additional_fields_fixedhash = array()
		) {
	global $entity, $id, $m2m_it, $cms_dbc, $input_size, $tpl_outer, $tpl_controlled, $debug_query;
	
	global $m2mtfcontrolledgrouped_groups_expander;
	global $m2mtfcontrolledgrouped_groups_expander_show;
	global $m2mtfcontrolledgrouped_groups_expanded;
	
	global $m2mtfcontrolledgrouped_outer_leftjoin_tables;
	global $m2mtfcontrolledgrouped_outer_leftjoin_fields;
	global $m2mtfcontrolledgrouped_outer_leftjoin_sorting;
	global $m2mtfcontrolledgrouped_outer_leftjoin_groupby;

	$ret = "";
	
	$msig = " //m2mtfcontrolledgrouped()";
//	pre("dict_table[$dict_table], value[" . pr($value) . "]"
//		. ", m2m_content_table[$m2m_content_table]"
//		. ", m2m_controlling_fixedhash[" . pr($m2m_controlling_fixedhash) . "]"
...
//		);
	...
	$tour_o2m_table = $entity;

	if (!isset($m2mtfcontrolledgrouped_groups_expander)) {
		$m2mtfcontrolledgrouped_groups_expander = "<div><a href=javascript:toggle_AllOuter()>Раскрыть / схлопнуть ответы всех команд</a></div>";
		$m2mtfcontrolledgrouped_groups_expander = "";
	}
	if (!isset($m2mtfcontrolledgrouped_groups_expanded)) {
		$m2mtfcontrolledgrouped_groups_expanded = true;
	}

	if (!isset($m2mtfcontrolledgrouped_outer_leftjoin_tables)) {
		$m2mtfcontrolledgrouped_outer_leftjoin_tables = "";
	}
	if (!isset($m2mtfcontrolledgrouped_outer_leftjoin_fields)) {
		$m2mtfcontrolledgrouped_outer_leftjoin_fields = "";
	}
	if (!isset($m2mtfcontrolledgrouped_outer_leftjoin_sorting)) {
		$m2mtfcontrolledgrouped_outer_leftjoin_sorting = "";
	}
	if (!isset($m2mtfcontrolledgrouped_outer_leftjoin_groupby)) {
		$m2mtfcontrolledgrouped_outer_leftjoin_groupby = "";
	}
	
	
	if (isset($tpl_outer) == false) {
		$tpl_outer = <<< EOT
<div style='float: left; margin: 0 2ex 2ex 0; padding: 1ex; border: 1px solid gainsboro'>
	<a href='javascript:ilayer_switch("${tour_o2m_table}_#GROUP_ID#")'>#GROUP_IDENT#</a>
	<div id='layer_${tour_o2m_table}_#GROUP_ID#' style='display:#INNER_STYLE_DISPLAY#; padding-left: 2ex'>
...
EOT;
	...
	$m2m_controlling_fixedhash = absorb_fixedhash($m2m_controlling_fixedhash);
	$query_outer = "select team.id, team.ident" . $m2mtfcontrolledgrouped_outer_leftjoin_fields
	...
	$query_outer = add_sql_table_prefix($query_outer);
	if ($debug_query) echo "m2mtfcontrolledgrouped query_outer [$query_outer]";

	$qa_outer = select_queryarray($query_outer);
	foreach ($qa_outer as $row_outer) {
		$questions_forTour_rendered = "";
		...
		$query_inner = "select q.id, q.ident, q.published, m2m.correct as m2m_correct, m2m.content as m2m_content" . $additional_fields_csv
			. " from $dict_table q"
		...
		$result = mysql_query($query_inner, $cms_dbc) or die("SELECT M2M_CONTROLLED_GROUPED_INNER failed:<br>$query_inner:<br>" . mysql_error($cms_dbc));
		...
		while ($row = mysql_fetch_assoc($result)) {
			...
			$row["m2m_correct"] = $row["m2m_correct"];
			$row["m2m_correct_checked"] = $row["m2m_correct"] == 1 ? "checked" : "";

			$row["GROUP_ENTITY"]	= $grouping_table;
			$row["GROUP_ID"]		= $row_outer["id"];
			$row["GROUP_IDENT"] 	= $row_outer["ident"];
			
			$questions_forTour_rendered .= hash_by_tpl($row, $tpl_controlled);
		}
		

		$row_outer["GROUP_ID"]	= $row_outer["id"];
		$row_outer["GROUP_IDENT"] = $row_outer["ident"];
		$row_outer["GROUP_CONTENT__CBX_TF"] = $questions_forTour_rendered;
		$row_outer["INNER_STYLE_DISPLAY"] = $m2mtfcontrolledgrouped_groups_expanded ? "block" : "none";
		

		$ret .= hash_by_tpl($row_outer, $tpl_outer);
	}
	
	$ret = $m2mtfcontrolledgrouped_groups_expander . $ret;
	$ret .= "<div style='clear: both'></div>";
	$ret .= $m2mtfcontrolledgrouped_groups_expander;
...
function m2mtf($dict_table,
	$fixed_hash = array ("_global:entity" => "_global:id"),
	$m2m_table = "_global", $item_tpl = "", $table_tpl = "") {
...
function select_table_all($table, $value = "_global", $sql_field = "ident", $tag_attr = ""
	...
	$msig = " //select_table_all()";

/*	pre("table[$table], value[" . pr($value) . "], sql_field[$sql_field], tag_attr[$tag_attr]"
		. ", fixed_hash[" . pr($fixed_hash) . "], forcezero_option[$forcezero_option], forcezero_evenifwasselected[$forcezero_evenifwasselected]"
		. " , graycomment[$graycomment]"
		, $msig);
*/
...
//	pre("$cache_key");
	...
	if (!isset($cache[$cache_key])) {
		...
		//pre($m2m_dependtable, "m2m_dependtable");
...
//				. sqlcond_fromhash($fixed_hash, "", " and ", " and ", TABLE_PREFIX)
		...
		$options = options_sql($query, $value, $forcezero_option, $forcezero_evenifwasselected, $msig);
		$cache[$cache_key] = $options;
//		pre("set to cache $cache_key");
	} else {
		//pre(" cache_key[$cache_key]:" . pr($cache[$cache_key]));
...
function select_queryro($name, $value, $default, $tag_attr = "") {
	$msig = " //select_queryro(name[$name])";
	$options = options_sql($default, $value, "", 0, $msig);
	return selectro($name, $options, $tag_attr);
}


function select_query($name, $value, $default, $tag_attr = "") {
	global $msg_tag_shortcut, $msg_bo_it_add, $msg_bo_it_change, $msg_bo_it_tolist;
	
	$msig = " //select_query(name[$name])";
	$options = options_sql($default, $value, "", 0, $msig);

	$ret = select($name, $options, $tag_attr);
	
	$input_type_wrapped = "&nbsp;&nbsp;&nbsp;&nbsp;";
	$select_fixed_suffix = "$name=$value";
	$select_fixed_suffix_and = "&$name=$value";
	//if ($value > 0) {
		if ($value > 0) $input_type_wrapped .= "<a href=$name-edit.php?id=$value{$select_fixed_suffix_and} target='_blank'>$msg_tag_shortcut $msg_bo_it_change</a>&nbsp;&nbsp;";
		$input_type_wrapped .= "<a href=$name-edit.php?{$select_fixed_suffix} target='_blank'>$msg_tag_shortcut $msg_bo_it_add</a>";
	//} else {
	//	$input_type_wrapped .= "<a href=$name.php{$select_fixed_suffix} target='_blank'>$msg_tag_shortcut $msg_bo_it_tolist</a>";
	//}

	return $ret . $input_type_wrapped;
}
...
function options_sql($query, $default, $forcezero_option = "", $forcezero_evenifwasselected = 0, $invoker = "INVOKER_UNKNOWN") {
...
//	echo "[$cms_dbc] / [$query]";
	...
	$query = add_sql_table_prefix($query);
	$result = mysql_query($query, $cms_dbc) or die("OPTIONS_SQL failed:<br>$query:<br>$invoker" . mysql_error($cms_dbc));
	$num_rows = mysql_num_rows($result);
...
function radio_table($table, $value = "_global", $sql_field = "ident", $tag_attr = "", $fixed_hash = array()) {
	...
	global $msg_bo_it_change, $msg_bo_it_add, $msg_bo_it_tolist, $msg_tag_shortcut;
...
<tr>
	<td align=right>
		<a href="$table_strict-edit.php?id=$value"	target="_blank">$msg_tag_shortcut $msg_bo_it_change</a>
		&nbsp;&nbsp;&nbsp;
		<a href="$table_strict-edit.php"			target="_blank">$msg_tag_shortcut $msg_bo_it_add</a>
		&nbsp;&nbsp;&nbsp;
		<a href="$table_strict.php"					target="_blank">$msg_tag_shortcut $msg_bo_it_tolist</a>
	</td>
</tr>
EOT;
	}
...
//	if ($value == "_global") $value = get_number($table);
	...
	if (isset($id)) {
		...
		$options = options_sql($query, $o2m_value, "", 0, $msig);
	}
