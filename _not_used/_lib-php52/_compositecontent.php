		...
		switch ($compositetype) {
			...
			case "tour":
				$options = optionscompositecontent_pgroupfromroot(
						$m2m_table, $m2m_fixed, $composite, $product_table, 0, $parent_id,
						$pgroup_table, $product_table, $supplier_table,
						$pgroup_levels_display, $level1_column_cnt, 1);
				break;
...
function optionscompositecontent_pgroupfromroot($m2m_table, $m2m_fixed, $composite,
	...
	$pgroup_table = "pgroup", $product_table = "product", $supplier_table = "supplier",
	$pgroup_levels_display = array(), $level1_column_cnt = 1, $get_down_products = 1, $product_to_pgroup_one2one = 1) {

	global $cms_dbc, $composite_inputtype, $pgroup_parentchain, $entity_fixed_list;
			
	global $compositecontent_select_pgroup_fixedhash;
	global $compositecontent_select_product_fixedhash;
	global $multicompositecontent_pgroup_expanded;
	global $multicompositecontent_product_expanded;
	...
	//pre("pgroup_table=[" . pr($pgroup_table) . "] array_keys($entity_fixed_list)[" . pr(array_keys($entity_fixed_list)) . "]");
	
	if (in_array($pgroup_table, array_keys($entity_fixed_list))) {
		$fixed_array_may_contain_parent_id = $entity_fixed_list[$pgroup_table];
		//pre("fixed_array_may_contain_parent_id=[" . pr($fixed_array_may_contain_parent_id) . "]");
	
		if (in_array("parent_id", $fixed_array_may_contain_parent_id)) {
			$pgroup_istree = 1;
			//pre("pgroup_istree=[" . $pgroup_istree . "]");
		}
	}
	/*pre("m2m_table=[" . pr($m2m_table) . "] m2m_fixed=[" . pr($m2m_fixed) . "] composite=[" . pr($composite) . "]"
		. " downtable=[$downtable] level=[$level] parent_id=[$parent_id] pgroup_table=[$pgroup_table] product_table=[$product_table] supplier_table=[$supplier_table]  pgroup_levels_display=[" . pr($pgroup_levels_display) . "] level1_column_cnt=[$level1_column_cnt] pgroup_parentchain=[" . pr($pgroup_parentchain) . "]"
...
//	$make_checkboxed_condition = in_array("pgroup", $composite);
	...
	if ($pgroup_istree == 1) {
		$compositecontent_select_pgroup_sqlcond =  sqlcond_fromhash($compositecontent_select_pgroup_fixedhash, "pg", " and ");
		...
		if ($product_to_pgroup_one2one == 1) {
...
EOT;
				...
				if ($pgroup_table == "tour") {
					$tpl = <<< EOT


<div style="padding-top: 3px; padding-bottom: 3px; padding-left: #PADDING-LEFT#em"><a href="javascript:pgroup_onclick('#OPTION_VALUE#')" #HREF_CLASS#>#IDENT#</a></div>
EOT;
...
//			$row["option_color"] = OPTIONS_COLOR_GRAY;
			...
			if (isset($multicompositecontent_pgroup_expanded) &&
...
EOT;
				...
				case "question":
					...
					if ($get_down_products == 1) {
						...
						$product_fixedhash = array($pgroup_table => $row["id"]);
						if (isset($compositecontent_select_product_fixedhash) &&
							is_array(array_keys($compositecontent_select_product_fixedhash))) {
							$product_fixedhash = array_merge($product_fixedhash, $compositecontent_select_product_fixedhash);
						}
						//pre("product_fixedhash=[" . pr($product_fixedhash) . "] ");
						$downtable_options = optionscompositecontent_productbyfixed (
									$m2m_table, $m2m_fixed, $composite, $product_fixedhash, $level + 1,
										$pgroup_table, $product_table, $supplier_table, $get_down_products);
					}
						
					if ($composite_inputtype == "multicheckbox") {
						$pgroup_id = $row["id"];
						$id_name = ($m2m_table == "m2m_product_fake4navigation")
							? "nav_pgroup_$pgroup_id"
							: "pgroup_$pgroup_id";
...
function optionscompositecontent_supplierbypgroup($m2m_table, $m2m_fixed, $composite,
	...
	global $cms_dbc, $composite_inputtype;
	global $compositecontent_select_supplier_fixedhash;
	...
	$compositecontent_select_supplier_sqlcond = sqlcond_fromhash($compositecontent_select_supplier_fixedhash, "s", " and ");
	...
	$query = "select pg.id as pgroup, s.id as supplier, s.ident, s.published"
		...
		. " where pg.id=$pgroup_id" . $compositecontent_select_supplier_sqlcond
		. " group by s.id"
...
function multicompositecontent_update($m2m_table, $value_arr,
			$composite = array("supplier", "pgroup"),
			$insert_absorbing_fixedhash = array("_global:entity" => "_global:id")) {
	global $entity, $id, $debug_query, $cms_dbc;
	global $multicompositecontent_insert_fixedhash;
	
	$ret = "";

//	pre($value_arr);

	$insert_absorbed_fixedhash = absorb_fixedhash($insert_absorbing_fixedhash);
	if (isset($multicompositecontent_insert_fixedhash) &&
		is_array(array_keys($multicompositecontent_insert_fixedhash))) {
		$insert_absorbed_fixedhash = array_merge($insert_absorbed_fixedhash, $insert_absorbed_fixedhash);
	}
//	pre("insert_absorbed_fixedhash[" . pr($insert_absorbed_fixedhash) . "] composite[" . pr($composite) . "]");
...
//		pre("composite_insert_value[" . pr($composite_insert_value) . "]");

		$composite_insert_hash = array();
		for($i=0; $i<sizeof($composite); $i++) {
			$ith_key = $composite[$i];
			if (in_array($i, array_keys($composite_insert_value))) {
				$ith_value = $composite_insert_value[$i];
			} else {
				if (in_array($ith_key, array_keys($insert_absorbed_fixedhash))) continue;
				$ith_value = get_number($ith_key);
			}
			$composite_insert_hash[$ith_key] = $ith_value;
		}
//		pre("composite_insert_hash[" . pr($composite_insert_hash) . "]");

		$insert_base = array("date_created" => "CURRENT_TIMESTAMP");
		$insert_hash = array_merge($insert_base, $insert_absorbed_fixedhash, $composite_insert_hash);
		
//		pre("insert_hash[" . pr($insert_hash) . "]");
...
function composite_itvalue($composite, $row) {
	$option_value = "";
	foreach($composite as $value) {
		if (isset($row[$value]) == false) {
			// ppgoption in product has product which is redundant?..
			continue;
		}
