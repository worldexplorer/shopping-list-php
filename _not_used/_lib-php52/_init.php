...
function gethash_bytplhash($tplhash, $absorb_in_cookie = 1, $get_first_from_cookie = 0, $checkboxes_array = array()) {
	...
	foreach ($tplhash as $tplkey => $tplvalue) {
		...
		if ($debug_cookies == 1) echo "[$tplkey] :: tplvalue=[$tplvalue] : _REQUEST=[$rvalue] : _COOKIE=[$cvalue]<br>";
		...
		if ($rvalue == "") {
			...
			if ($absorb_in_cookie == 1 && $rvalue != $cvalue) {
				prefixed_setcookie($tplkey, $rvalue, time() + 60*60*24*30);
				if ($debug_cookies == 1) echo "[$tplkey] :: setting cookie to [$rvalue]<br>";
			}
		}
		
		if (in_array($tplkey, $checkboxes_array)) {
//			pre("in_array($tplkey, " . pr($checkboxes_array) . ") = 1");
//	by default - from template or cookies !!! why zero???
//			if ($mode == "" && $value == "") $value = 0;
			if ($mode == "update") {
				$value = get_number($tplkey);
