...
//		pre($entity);
//		pre($no_submenu_list);
//		pre("in_array($entity, " . pr($no_submenu_list) . ") = " . (in_array($entity, $no_submenu_list)+0) );

		if (in_array($entity, $no_submenu_list) == false) submenu_for_entity($entity);
		break;
...
function submenu_for_entity($entity, $href_basepage = "_global", $href_fixedhash = array()) {
	...
	if (isset($entity_fixed_list[$entity]) == false) return;
...
//		$txt = $entity_list[$dependant_entity];
		$txt = ident_from_ident_new($dependant_entity);
...
//			$options = options_sql($query, $current);
			...
			$entity_name_utf = $entity_list[$dependant_entity];
			$entity_name_utf_lowercase = strtolower($entity_name_utf);
			$forcezero_option = ($fixed_getfirstfromdb == 0) ? " - $msg_submenu_all " . $entity_name_utf . " - " : "";
...
//			echo "The string '$findme' was found in the string '$mystring' and exists at position $pos";
			...
			$options = options_sql_tree($dependant_entity, $current, "ident", 1
				, 1, 0, " - $msg_submenu_all " . $entity_name_utf . " - ", 1
			);
		}
