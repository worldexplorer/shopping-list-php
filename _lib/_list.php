...
//		pre ($column_params);
		...
		if ($column_type == "cnt") {
			// mzgb: "game" is already used, shortening "game-HASHKEY_ALREADY_USED"
			$table_column = makestrict($table_column, "-");
...
//			echo "entity_list[" . makestrict($table_column) . "] = [" . $entity_list[makestrict($table_column)] . "]";
		...
		switch ($column_type) {
			case "sernoupdown":
				$header_tmp = "<th style='width:3ex'>" . $header_txt . "</th><th style='width:5ex'></th>";
...
//EOT;
				$item_tmp = <<< EOT
<td><a href="#ENTITY#-edit.php?id=#ID#{$updown_suffix}">#$table_column#</a></td>
...
EOT;
			...
			case "cnt":
				...
				$item_tmp = <<< EOT
<td align=center nowrap><a href="$hrefto">$param1 #${table_column_upper}_CNT#</a></td>
EOT;
				break;
...
EOT;
			...
			default:
				pre("NO_RENDERING_HANDLER_FOR_COLUMNTYPE[$column_type]");
				break;
		}
...
// товары через m2m, вывести количество товаров дл¤ группы, два вызова аналогичны:
// pgroup.php: "~3" => array("“оваров", "depend", "товаров: ")
...
//if ($items_updated == 1) $errormsg .= " »нформаци¤ сохранена";
//if ($items_deleted == 1) $errormsg .= " Ёлементы удалены";
...
//							. $onegroup . ">" . $onegroup . "</a>";
			...
			case "groupconcat_to_ahref":
				$param1 = (isset($column_params[2])) ? $column_params[2] : "";	//style / th width
				$param2 = (isset($column_params[3])) ? $column_params[3] : "";
				$param3 = (isset($column_params[4])) ? $column_params[4] : "";
				
				$tpl_single		= $param2;
				$tpl_multiple	= $param3;
				$groupconcat_to_ahrefEach = groupconcat_to_ahref($row, $table_column, $tpl_single, $tpl_multiple);
				$row[$table_column] = $groupconcat_to_ahrefEach;

				//pre($groupconcat_to_ahrefEach, "groupconcat_to_ahrefEach");
				//pre($row, "row");
				//pre(hash_by_tpl($row, $table_column), "row[table_column]");
...
*/				
			...
			default:
				//pre("NO_PREUPDATE_HANDLER_FOR_COLUMNTYPE[$column_type]");
				break;
		}
	}
...
<tr>
	<td colspan=2>
		...
		<tr valign=bottom>
			<!--td width=15%><?=$topline_left_fromSubmenu?></td>
			<td width=70%-->
				<table cellpadding=0 cellspacing=0 border=0 align=center style="float:left">
				<tr valign=top>
...
<? } ?>
			...
			<!--/td>
			<td width=15%-->
			<? if ($topline_right_fromSubmenu_show) { ?>
				<div style="width: 46ex; float: right">
				<form id=showColumns>
					<input type=radio name=question_showColumns value=viewing id=showColumns_forViewing checked
						onchange="javascript:showColumns_changed('<?=$entity?>', <?=$entity?>_columnIndexes_forViewing, <?=$entity?>_columnIndexes_forEditing)" />
							<label for=showColumns_forViewing>просмотр</label>
					<input type=radio name=question_showColumns value=editing id=showColumns_forEditing
						onchange="javascript:showColumns_changed('<?=$entity?>', <?=$entity?>_columnIndexes_forViewing, <?=$entity?>_columnIndexes_forEditing)" />
							<label for=showColumns_forEditing>редактирование</label>
					<input type=radio name=question_showColumns value=allColumns id=showColumns_all
						onchange="javascript:showColumns_changed('<?=$entity?>', <?=$entity?>_columnIndexes_allColumns, <?=$entity?>_columnIndexes_allColumns)" />
							<label for=showColumns_all>все колонки</label>
				</form>
				</div>
...
<? if ($table_has_editables == 1 && $no_savebutton == 0) { ?>
		<?=$f5_near_save?>
		<input style="width: 7em" type=button value="<?=$msg_bo_save?>" onclick="javascript:onsave_<?=$entity?>()" id="btn_save_list">
<? } ?>


<? if ($no_del == 0) { ?>
<? if (!in_array($entity, $no_delentity_list)) { ?>
		<input style="width: 6em" type=button value="<?=$msg_bo_delete?>" onclick="javascript:ondelete_<?=$entity?>()" id="btn_deleteChecked_list">
		<input type=checkbox id="cb_checkall_4delete" name="cb_checkall_4delete" onclick="javascript:checkall_4delete()" title="<?=$msg_bo_delete_all_tip?>" id="cbx_checkAll_list">
