...
<!--form method=get-->
<form enctype="multipart/form-data" method="<?=$form_edit_method?>" id="form_edit" name="form_edit">
	<input type=hidden name=mode value=update>
<? if ($no_backoffice_fields_in_edit_form == 0) { ?>
...
if ($id > 0) {
	...
	if (function_exists($entity_before_wrapping_function)) {
		$db_values_array = $entity_before_wrapping_function($db_values_array);
	}
}
...
foreach ($entity_fields as $name => $it_params) {
	...
	$param9			= (isset($it_params[11])) ? $it_params[11] : "";
	$param10		= (isset($it_params[12])) ? $it_params[12] : "";
...
//			echo "[$field_txt: $input_type($name, $entity|$id, $default, $param1)]: ";
			$layer_open_html = layer_open($name, $field_txt);
			$wide_open_html = wide_open($name, $field_txt);
			//$back_row = $input_type($name, array ($entity => $id), $default, $param1, $param2, $param3, $param4);

			$m2m_content_table 				= $default;
			$m2m_controlling_table 			= $param1;
			$m2m_controlling_fixedhash 		= $param2;
			//$m2m_team_question_fixed4insert	= $param3;
			$additional_fields_fixedhash 	= $param4;
			
			$back_row = m2mtfcontrolled($name, array ($entity => $id),
				$m2m_content_table, $m2m_controlling_table,
				$m2m_controlling_fixedhash,
...
//			echo "[$field_txt: $input_type($name, $entity|$id, $default, $param1)]: ";
			...
			$m2m_controlling_table 			= $param1;
			$m2m_controlling_fixedhash 		= $param2;
			$grouping_table 				= $param3;
			//$additional_fields_fixedhash 	= $param4;
			$additional_fields_fixedhash 	= $param5;
			
			$back_row = m2mtfcontrolledgrouped($name, array ($entity => $id),
				$m2m_content_table, $m2m_controlling_table,
				$m2m_controlling_fixedhash, $grouping_table,
				$additional_fields_fixedhash);
...
//			echo "[$field_txt: $input_type($name, $default, $param1, $param2, $param3, $param4, $param5, $param6, $param7)]: ";
		...
		case "multicompositecontent":
			//echo "[$field_txt: $input_type($name, $default, $param1, $param2, $param3, $param4, $param5, $param6, $param7)]: ";
			/*if ($param10 != "") {
				$input_type_wrapped = $input_type($name, $default, $param1, $param2, $param3, $param4, $param5, $param6, $param7, $param8, $param9, $param10);
			} else if ($param9 != "") {
				$input_type_wrapped = $input_type($name, $default, $param1, $param2, $param3, $param4, $param5, $param6, $param7, $param8, $param9);
			} else*/ if ($param8 != "") {
				$input_type_wrapped = $input_type($name, $default, $param1, $param2, $param3, $param4, $param5, $param6, $param7, $param8);
			} else if ($param7 != "") {
				$input_type_wrapped = $input_type($name, $default, $param1, $param2, $param3, $param4, $param5, $param6, $param7);
			} else if ($param6 != "") {
				pre("$field_txt: $input_type(compositetype[$name], m2m_table[$default], composite[$param1], parent_id[$param2], html_table_width[$param3], pgroup_table[$param4], product_table[$param5], supplier_table[$param6], pgroup_levels_display[$param7]), level1_column_cnt[$param8] ]: ");
...
//			echo "[$field_txt: $input_type($default, $param1, $param2, $param3, $param4, $param5, $param6)]: ";
			...
			if (!isset($multicompositeiccontent_wrap_into_layer_open)) $multicompositeiccontent_wrap_into_layer_open = true;
			...
			$icwhose_row = array("ident" => "ICWHOSE_NOT_SPECIFIED_BY_[$entity-edit.php]");
				
			if (is_numeric($param3)) {
				if ($param3 > 0) {
					$icwhose_row = select_entity_row(array("id" => $param3), "icwhose");
				}
			} else {
			...
			if (isset($icwhose_row["id"]) == false) {
				...
				echo "multicompositeiccontent(m2m_table[$default]"
					. " fixed_hash[" . pr($param1) . "] "
					. " absorbing_fixedhash[" . pr($param2) . "] "
					. " icwhose_id[$icwhose_id] "
					. " read_only[$read_only] "
					. " ignore_jsv_finally[$param3] "
					. " form_name[$param4])";
			} else {
				...
				if ($multicompositeiccontent_wrap_into_layer_open == false) {
					$back_row = <<< EOT
...
//				}
				...
				if ($multicompositeiccontent_wrap_into_layer_open) {
					$ondblclick = <<< EOT
...
//			echo "[$field_txt: $input_type($name, $value, $default, $param1, $param2)]: ";
		...
		case "boolean_hash":
			if ($default == "") $default = array("invalid" => "options_hash");
// 			echo "[boolean_hash: boolean_hash(name=$name, value=$value"
// 				. ", default=$default, opt_hash=" . pr($param1)
// 				. ", graycomment=$param2, tag_attr=$param3)]: ";
			$input_type_wrapped = boolean_hash($name, $value, $default, $param1, $param2, $param3);
			$back_row = back_row($input_type_wrapped, $field_txt, $name);
			break;
...
//			$ahref_wrapped = $input_type($name, $value, $default);
			...
			// mzgb: "question" is already used, shortening "question-HASHKEY_ALREADY_USED"
			$name = makestrict($name, "-");
			...
			// "&game=2" for  http://localhost:8002/mzgb/backoffice/tour-edit.php?id=8&game=2
			$ahref_fixed_suffix_and = hrefsuffix_fromhash($fixed_hash, "&");
			$ahref_wrapped = "<a href='$name.php?$entity=$id$ahref_fixed_suffix_and'>$ahref_relatedto</a>";
//			pre($ahref_wrapped);
...
//				pre($entity_fixed_list[$name]);
			...
			if ($id > 0) {
				if ($value > 0) {
					$input_type_wrapped .= "<a href=$tree_table-edit.php?id=$value{$select_fixed_suffix_and} target='_blank'>$msg_tag_shortcut $msg_bo_it_change</a>&nbsp;&nbsp;";
				}
...
if ($id == 0) {
	...
	if (isset($entity_list_single_savebutton[$entity])) {
		$button_msg_utf = $entity_list_single_savebutton[$entity];
		$button_msg_utf_lowercased = strtolower($button_msg_utf);
		$button_msg .= " " . $button_msg_utf;
	}
}
...
<? if ($no_bottomline == 0) {?>
<tr><td colspan=2 align=center height-disabled=60>
	<? if ($no_savebutton == 0) { ?>
