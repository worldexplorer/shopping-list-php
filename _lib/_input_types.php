...
function boolean_hash($name, $value = "_global", $default = "_global"
		, $opt_hash = array(0 => "no", 1 => "yes")
		, $graycomment = "", $tag_attr = "") {
	$ret = "";
	$gray = OPTIONS_COLOR_GRAY;

//	pre("boolean_hash($name, $value, " . pr($opt_hash) . ", $tag_attr");
//	if ($value == "_global") $value = get_number($name);
	if ($value == "") $value = $default;
//	if ($value == "") $value = absorb_variable($default);
...
/*
		$ret .= <<< EOT
<div><input type="radio" name="$name" id="${name}_rb_$key" value="$key" $checked $tag_attr>
<label for="${name}_rb_$key">$hashvalue</label></div>
EOT;
*/
		$ret .= <<< EOT
<tr>
	<td><input type="radio" name="$name" id="${name}_rb_$key" value="$key" $checked $tag_attr></td>
	<td width=3></td>
	<td><label for="${name}_rb_$key" $tag_attr>$hashvalue</label></td>
</tr>
EOT;
	}
...
<tr>
	<td>
		<table cellspacing=0 cellpadding=5 style="border:1px solid $gray" align=left>
		<tr valign=middle>
			<td>
				<table cellspacing=0 cellpadding=0>
				$ret
	...
	<td valign=bottom
		style='color:$OPTIONS_COLOR_GRAY; margin-left: 2ex; float: right'>
		$graycomment
	</td>
</tr>
</table>
...
function ro($name, $value, $default = "") {
	global $db_values_array, $msg_bo_empty;
...
//	global $input_size, $layer_inside, $db_values_array;
	global $input_size, $layer_inside, $msg_bo_empty;
...
function checkbox_ro($name, $value, $default = "", $graycomment = "", $tag_attr = "") {
	...
	if ($graycomment != "") $ret .= "$before_gray_space<font style='color: " . OPTIONS_COLOR_GRAY . "'>$graycomment</font>";
	return $ret;
}
