...
/*
 * img_resize
 * get input file name, resize it and crop if needed. Add specified padding.
 * abspath - absolute path for source and destination files
 * src_fname - source file name
 * dst_w - destination width
 * dst_h - destination height. Will do uniform scale in case of dst_w || dst_h = 0
 * dst_fname - destination file name
 * quality - jpeg compression quality: 100 - no compression, 50 - very poor
 * crop_from_X - crop image from this x column
 * crop_from_Y - crop image from this y row
 * padding - add transparent rows, format is similar to css: "top right bottom left" in pixels. "10px 5px" - top 10, right 5; "10px" - top, right, bottom, left 10;
 * use_greater_size - [1, 0] when dst_w or dst_h = 0 use greater input size for uniform scale.
 */
...
// Precalculations in case of insufficient input
			if($dst_h == 0 && $dst_w == 0){
				...
				//Uniform scale, for given height
				$dst_h = floor($h * $dst_w / $w);
			} else if($dst_w == 0){
				//Uniform scale, for given width
				$dst_w = floor($w * $dst_h / $h);
			} else {
				//given two target dims will scale and crop (if appropriate)
...
//		, $tpl_nex = "<img src='#UPLOAD_RELPATH#/#ENTITY#/default.gif' alt='#IMG_NEX_DEBUGMSG#' class=imgfloat border=1>"
	...
	global $cms_dbc, $upload_relpath, $upload_abspath, $debug_query, $autoresize_imghash, $datetime_fmt;
...
//			pre($ar_img_row);	// blind merging
			...
			$ar_img_row["IMG_RESIZABLE_BYEXTENSION"] = 1;
			if (img_exists($img_row, $field_from) == 1
					// commented out to get unresized pics as well && ($ar_width > 0 || $ar_height > 0)
				) {
				$ar_img_row["img_id"] = $img_row["id"];
//				pre($img_row);
